package com.fabiocompany.supermercadosdeltaplus.app.config.auth.rememberme;
...
public abstract class AbstractRememberMeServicesCustom
	...
	protected AbstractRememberMeServicesCustom(String key, UserDetailsService userDetailsService,
	 ...
	 * its value. The cookie is searched for by name and also by matching the
	 * context path to the cookie path.
	 *
	 * @param request
	 ...
	 * <p>
	 * By default it will create a <tt>RememberMeAuthenticationToken</tt>
	 * instance.
	 *
	 * @param request
	 *            the original request. The configured
	 *            <tt>AuthenticationDetailsSource</tt> will use this to build
	 *            the details property of the returned object.
	 * @param user
	 *            the <tt>UserDetails</tt> loaded from the
	 *            <tt>UserDetailsService</tt>. This will be stored as the
	 *            principal.
	 *
	 * @return the <tt>Authentication</tt> for the remember-me authenticated
	 *         user
	 */
	 ...
	 * <p>
	 * By default the cookie will be secure if the request is secure. If you
	 * only want to use remember-me over HTTPS (recommended) you should set this
	 * property to {@code true}.
	 *
	 * @param useSecureCookie
	 *            set to {@code true} to always user secure cookies,
	...
	/**
	 * Sets the strategy to be used to validate the {@code UserDetails} object
	 * obtained for the user when processing a remember-me cookie to
	 * automatically log in a user.
	 *
	 * @param userDetailsChecker
	 *            the strategy which will be passed the user object to allow it
	 *            to be rejected if account should not be allowed to
	 *            authenticate (if it is locked, for example). Defaults to a
